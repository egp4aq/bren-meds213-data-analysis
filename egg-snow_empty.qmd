...
When we look at our egg data, we see that there is this column egg_num that gives an arbitrary number to the egg in that nest. We want there to only be one row per nest, rather than egg so we can join this table with nest. 
...
eggs_sum <- eggs_data %>%
  ...
  summarize(Egg_count = max(Egg_num)) %>% # decided to use max number of eggs, but could have counted the number of rows for each grouped observation
  ungroup()
```

Then we can join with the nest table to bring the species information:
Even though we would probably be fine to just join on nest_ID and year, we are also going to join on site. There is no problem with being extra safe
...
snowcover_data_fixed <- snowcover_data_fixed %>%
  mutate(Date2 = ifelse(is.na(Date == "8&9 june 6"), ymd("08/06/06"), Date))
```
